    ...
    for filename in os.listdir(provider_dir):
        if filename.endswith(".py") and not filename.startswith("__") and filename != "base.py":
            ...
            for item_name in dir(module):
                ...
                if isinstance(item, type) and issubclass(item, LLMProvider) and item is not LLMProvider:
                    instance = item()
...
async def analyze_code(request: AnalysisRequest):
    if request.provider not in llm_providers:
        raise HTTPException(status_code=400, detail=f"LLM Provider '{request.provider}' not supported.")
    ...
    if request.language == "python":
        try:
            async with httpx.AsyncClient() as client:
                response = await client.post("http://python_analyzer:8001/analyze", json={"code": request.code})
                response.raise_for_status()
                analysis_result = response.json()["analysis"]
        except httpx.RequestError as exc:
            raise HTTPException(status_code=500, detail=f"Error communicating with Python Analyzer: {exc}")
        except httpx.HTTPStatusError as exc:
            raise HTTPException(status_code=exc.response.status_code, detail=f"Python Analyzer error: {exc.response.text}")
