...
def _detect_issues(code: str, lines: List[str]) -> List[str]:
    ...
    if "system(" in code or "`" in code:
        issues.append(
            "🔐 Security: System calls detected - review for command injection vulnerabilities"
        )
    ...
    if long_lines:
        issues.append(
            f"📏 Style: Long lines detected (lines: {', '.join(map(str, long_lines[:3]))}{'...' if len(long_lines) > 3 else ''})"
        )
    ...
    if re.search(r"if\s+.*\.nil\?", code):
        issues.append(
            "♻️ Ruby Style: Consider using safe navigation operator (&.) instead of nil checks"
        )
    ...
    if code.count("class") > 0:
        ...
        if class_line_count > 50:
            issues.append(
                "🏗️ SOLID: Large class detected - violates Single Responsibility Principle"
            )
    ...
    if magic_numbers:
        issues.append(
            "🔢 Code Quality: Magic numbers detected - consider using named constants"
        )
